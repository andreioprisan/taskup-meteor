...
DUET.SidebarView = function () {
    ...
    var options = {userName: DUET.my.first_name + ' ' + DUET.my.last_name};
    this.initialize(options);
};
...
DUET.TaskListView.prototype.postRenderProcessing = function () {
    if (!this.collection.models.length)
        this.showMessage('No tasks for this project yet. Type below to create a task.');
...
DUET.ListView = function (data, map, selectedId) {
    ...
    var clientProjectMapping = new Array();
    ...
    data.models = $.map(data.models, function (item, key) {
		...
		if (clients[mappedItem.clientId] !== undefined &&
			clients[mappedItem.clientId] == mappedItem.clientName) {
			mappedItem.showClientName = false;
		} else {
			if (mappedItem.type == "project") {
				mappedItem.showClientName = true;
			} else {
				mappedItem.showClientName = false;				
			}
		}
        ...
        if (clientProjectMapping[mappedItem.clientId] == undefined) {
            clientProjectMapping[mappedItem.clientId] = [];
        }
        clientProjectMapping[mappedItem.clientId].push(mappedItem.id);
    ...
    console.log(clients);
    console.log(clientProjectMapping);
